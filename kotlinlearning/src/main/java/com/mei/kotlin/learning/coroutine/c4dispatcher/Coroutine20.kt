...
/**
 * 协程与线程之间的关系
 * 协程上下文与分发器(Coroutine Context 与Dispatcher)
 * 协程总是会在某个上下文中执行，这个上下文实际上是由Corout ineContext类型的-个实例来表示的，该实例是由Kotlin标准库来定义的
 * 协程上下文本质，上是各种元素所构成的-个集合。其中,主要的元素包括协程的Job,以及分发器。
 ...
 *
 * 所有的协程构建器(coroutine builder) 如1 aunch和async都会接收一个可选的CoroutineContext参数，
 * 该参数可用于显式指定新协程所运行的分发器以及其他上下文元素。
 *
 * 程序分析:
 * 1.当通过launch来启动协程并且不指定协程分发器时，它会继承启动它的那个CoroutineScope的上下文与分发器。
 * 对于该示例来说，它会继承runBlocking的上下文，而runBlocking则是运行在main线程当中。
 * 2. Dispatchers.Unconfined是一种很特殊的协程分发器， 它在该示例中也是运行在main线程中，但实际上，
 * 其运行机制与不指定协程分发器时是完全不同的
 * 3. Dispatchers.Default是默认的分发器，当协程是通过GlobalScope来启动的时候，它会使用该默认的分发器来启动协程，
 * 它会使用一个后台的共享线程池来运行我们的协程代码。因此 launch(Dispatchers.Default)等价于GlobalScope.launch{}
 * 4. Executors.newSingleThreadExecutor().asCoroutineDispatcher)创建一个单线程的线程池，
 * 该线程池中的线程用来执行我们所指定的协程代码；在实际的开发中，使用专门的线程来执行协程代码代价是非常高的，
 * 因此在协程代码执行完毕后，我们必须要释放相应的资源，这里就需要使用close方法来关闭相应的协程分发器，从而释放掉资源；
 * 也可以将该协程分发器存储到一个顶层变量当中，以便在程序的其他地方进行复用
 */
